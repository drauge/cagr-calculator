...
const chartSeries = [
  { key: "startBalance", label: "ETF start balance", color: "#1f77b4", enabled: true },
  ...
  { key: "etfEndValueAfterTaxReal", label: "Inflation-adjusted ETF value", color: "#bcbd22", enabled: false },
  { key: "endOfYearNetValueReal", label: "Inflation-adjusted net worth", color: "#7f7f7f", enabled: false },
  { key: "mortgageDebtJan1", label: "Mortgage debt Jan 1", color: "#8c564b", enabled: false },
  { key: "propertyValueJan1", label: "Property value Jan 1", color: "#17becf", enabled: false },
];
...
function monthlyDateByPaymentDay(anchorDateString, monthOffset, paymentDay) {
  ...
  const target = new Date(Date.UTC(anchor.getUTCFullYear(), anchor.getUTCMonth() + monthOffset, 1));
...
function hasPropertySale(model) {
  return Boolean(model.propertySaleDate);
}

function isOnOrAfterSale(model, dateString) {
  return hasPropertySale(model) && dateString >= model.propertySaleDate;
}

function isBeforeSale(model, dateString) {
  return !hasPropertySale(model) || dateString < model.propertySaleDate;
}
...
function addPropertyAppreciationRow({ effectiveFrom = "2025-01-01", appreciation = 0 } = {}) {
  const row = document.querySelector("#propertyAppreciationTable tbody").insertRow();
  addInputCell(row, "date", effectiveFrom);
  addInputCell(row, "number", appreciation, { step: "0.1" });
  addRemoveButton(row);
}

function addInflationRow(tableSelector, { effectiveFrom = "2025-01-01", inflation = 0 } = {}) {
  const row = document.querySelector(`${tableSelector} tbody`).insertRow();
  addInputCell(row, "date", effectiveFrom);
  addInputCell(row, "number", inflation, { step: "0.1" });
  addRemoveButton(row);
}
...
function readModel() {
  ...
  return {
    ...
    mortgageStartDate: document.getElementById("mortgageStartDate")?.value || "2025-08-25",
    firstPaymentDate: document.getElementById("firstPaymentDate")?.value || "2025-09-08",
    repaymentStartDate: document.getElementById("repaymentStartDate")?.value || "2025-11-06",
    paymentDay: Math.max(1, Math.min(31, Math.round(inputNumber("paymentDay") || 6))),
    durationMonths: Math.max(1, Math.round(inputNumber("durationMonths"))),
    bankMargin: inputPct("bankMargin"),
    propertySaleDate: document.getElementById("propertySaleDate")?.value || "", 
    ...
    propertyAppreciationSchedule: tableRows("#propertyAppreciationTable")
      .map(([effectiveFrom, appreciation]) => ({ effectiveFrom, appreciation: n(appreciation) / 100 }))
      .filter((r) => r.effectiveFrom)
      .sort((a, b) => a.effectiveFrom.localeCompare(b.effectiveFrom)),

    nlInflationSchedule: tableRows("#nlInflationTable")
      .map(([effectiveFrom, inflation]) => ({ effectiveFrom, inflation: n(inflation) / 100 }))
      .filter((r) => r.effectiveFrom)
      .sort((a, b) => a.effectiveFrom.localeCompare(b.effectiveFrom)),

    ltInflationSchedule: tableRows("#ltInflationTable")
      .map(([effectiveFrom, inflation]) => ({ effectiveFrom, inflation: n(inflation) / 100 }))
      .filter((r) => r.effectiveFrom)
      .sort((a, b) => a.effectiveFrom.localeCompare(b.effectiveFrom)),
...
function activePropertyAppreciation(model, dateString) {
  let active = model.propertyAppreciationSchedule?.[0]?.appreciation ?? 0;

  for (const row of model.propertyAppreciationSchedule || []) {
    if (row.effectiveFrom <= dateString) active = row.appreciation;
  }

  return active;
}

function daysBetweenDatesInclusiveStart(startDateString, endDateString) {
  return Math.max(0, daysBetween(startDateString, endDateString));
}

function compoundFactorFromSchedule(schedule, dateString, valueKey) {
  if (!schedule || !schedule.length) return 1;

  const ordered = [...schedule].sort((a, b) => a.effectiveFrom.localeCompare(b.effectiveFrom));
  let factor = 1;
  let currentDate = ordered[0].effectiveFrom;
  let currentRate = ordered[0][valueKey];

  if (dateString <= currentDate) return factor;

  for (let i = 1; i < ordered.length; i += 1) {
    const nextDate = ordered[i].effectiveFrom;
    if (nextDate >= dateString) break;

    const days = daysBetweenDatesInclusiveStart(currentDate, nextDate);
    factor *= Math.pow(1 + currentRate, days / 365.25);

    currentDate = nextDate;
    currentRate = ordered[i][valueKey];
  }

  const remainingDays = daysBetweenDatesInclusiveStart(currentDate, dateString);
  factor *= Math.pow(1 + currentRate, remainingDays / 365.25);

  return factor;
}

function propertyValueOnDate(model, dateString) {
  if (isOnOrAfterSale(model, dateString)) return 0;

  const schedule = model.propertyAppreciationSchedule || [];
  return model.propertyValue * compoundFactorFromSchedule(schedule, dateString, "appreciation");
}

function inflationFactorOnDate(schedule, dateString) {
  return compoundFactorFromSchedule(schedule || [], dateString, "inflation");
}
...
function buildMortgageSchedule(model) {
  ...
  for (let i = 0; i < paymentDates.length; i += 1) {
    ...
    const interest = isPrincipalPeriod
      ...
      : balanceStart * annualRate * days / 365;
...
function mortgageSnapshotJan1(mortgageSchedule, year, model) {
  const janDateString = `${year}-01-01`;
  const jan1 = dateStringToUtc(janDateString);

  if (isOnOrAfterSale(model, janDateString)) {
    return {
      balanceStart: 0,
...
function mortgageDebtOnDate(mortgageSchedule, dateString, model) {
  if (isOnOrAfterSale(model, dateString)) return 0;

  const target = dateStringToUtc(dateString);
  const lastBeforeOrOnDate = mortgageSchedule
    .filter((r) => dateStringToUtc(r.dateString) <= target)
    .sort((a, b) => a.dateString.localeCompare(b.dateString))
    .at(-1);

  if (lastBeforeOrOnDate) return lastBeforeOrOnDate.balanceEnd;

  return model.loanAmount;
}

function propertySaleProceeds(model, mortgageSchedule) {
  if (!hasPropertySale(model)) return 0;

  const valueAtSale = propertyValueOnDate(model, model.propertySaleDate);
  const debtAtSale = mortgageDebtOnDate(mortgageSchedule, model.propertySaleDate, model);

  return Math.max(0, valueAtSale - debtAtSale);
}
...
function calculateBox3(model, etfJan1, debtJan1, propertyValueJan1, year) {
  ...
  const assets = etfJan1 + propertyValueJan1;
  ...
  const foreignReturnBeforeShare = propertyValueJan1 * model.otherAssetsReturn - debtReturnDeduction;
...
function simulate(model) {
  ...
  const saleProceeds = propertySaleProceeds(model, mortgageSchedule);
  const monthlyEtfReturn = Math.pow(1 + model.etfGrossReturn, 1 / 12) - 1;

  let etf = model.etfStartingValue;
  let saleProceedsAllocated = false;
  const annual = [];
  const cashFlows = [{ date: ymToDate(model.projectionStartYear, 1), amount: -model.etfStartingValue }];
  ...
  for (let year = model.projectionStartYear; year <= model.projectionEndYear; year += 1) {
    ...
    const valuationDate = `${year}-01-01`;
    const propertyValueJan1 = propertyValueOnDate(model, valuationDate);
    const nlInflationFactorJan1 = inflationFactorOnDate(model.nlInflationSchedule, valuationDate);
    const ltInflationFactorJan1 = inflationFactorOnDate(model.ltInflationSchedule, valuationDate);
    const box3 = calculateBox3(model, etfJan1, mortgageJan1.balanceStart, propertyValueJan1, year);
    ...
    for (let month = 1; month <= 12; month += 1) {
      ...
      if (
        hasPropertySale(model) &&
        !saleProceedsAllocated &&
        dateStringToUtc(`${year}-${String(month).padStart(2, "0")}-01`) >= dateStringToUtc(model.propertySaleDate)
      ) {
        etf += saleProceeds;
    ...
    const etfEndValueAfterTax = etf;

    const eoyDate = `${year}-12-31`;
    const propertyValueEoy = propertyValueOnDate(model, eoyDate);
    const mortgageDebtEoy = mortgageDebtOnDate(mortgageSchedule, eoyDate, model);
    const propertyEquityEoy = Math.max(0, propertyValueEoy - mortgageDebtEoy);

    const propertyEquityJan1 = Math.max(0, propertyValueJan1 - mortgageJan1.balanceStart);

    const nlInflationFactorEoy = inflationFactorOnDate(model.nlInflationSchedule, eoyDate);
    const ltInflationFactorEoy = inflationFactorOnDate(model.ltInflationSchedule, eoyDate);

    const etfEndValueAfterTaxReal = nlInflationFactorEoy > 0 ? etfEndValueAfterTax / nlInflationFactorEoy : etfEndValueAfterTax;
    const propertyEquityEoyReal = ltInflationFactorEoy > 0 ? propertyEquityEoy / ltInflationFactorEoy : propertyEquityEoy;

    const endOfYearNetValue = etfEndValueAfterTax + propertyEquityEoy;
    const endOfYearNetValueReal = etfEndValueAfterTaxReal + propertyEquityEoyReal;
    ...
    annual.push({
      ...
      etfEndValueAfterTax,
      etfEndValueAfterTaxReal,
      propertyEquityEoy,
      propertyEquityEoyReal,
      propertyValueEoy,
      mortgageDebtEoy,
      saleProceeds,
      endOfYearNetValue,
      endOfYearNetValueReal,
      etfJan1,
  ...
  const simpleCagr = totalInvested > 0 ? Math.pow(etf / totalInvested, 1 / model.projectionYears) - 1 : null;
  ...
  return {
    ...
    mortgageSchedule,
    finalEtf: etf,
    finalNetWorth: annual.length ? annual[annual.length - 1].endOfYearNetValue : etf,
    finalEtfReal: annual.length ? annual[annual.length - 1].etfEndValueAfterTaxReal : etf,
    finalNetWorthReal: annual.length ? annual[annual.length - 1].endOfYearNetValueReal : etf,
    totalInvested,
...
function renderChart(rows) {
  ...
  enabled.forEach((series) => {
    ...
    svg.appendChild(svgEl("path", { d: linePath(points), class: "chart-line", stroke: series.color }));

    points.forEach((p) => {
      const circle = svgEl("circle", { cx: p.x, cy: p.y, r: 3.5, fill: series.color, class: "chart-point" });
...
function render(result) {
  ...
  document.getElementById("finalEtfReal").textContent = fmtEUR.format(result.finalEtfReal);
  document.getElementById("finalNetWorth").textContent = fmtEUR.format(result.finalNetWorth);
  document.getElementById("finalNetWorthReal").textContent = fmtEUR.format(result.finalNetWorthReal);
  ...
  result.annual.forEach((r) => {
    ...
    const values = [
      ...
      fmtEUR.format(r.etfEndValueAfterTax),
      fmtEUR.format(r.etfEndValueAfterTaxReal),
      fmtEUR.format(r.mortgageDebtJan1),
      fmtEUR.format(r.accumulatedCapitalJan1),
      fmtEUR.format(r.propertyValueJan1),
      fmtEUR.format(r.endOfYearNetValue),
      fmtEUR.format(r.endOfYearNetValueReal),
...
function downloadCsv() {
  ...
  const rows = [
    ...
    ...result.annual.map((r) => [
      ...
      r.etfEndValueAfterTax.toFixed(2),
      r.etfEndValueAfterTaxReal.toFixed(2),
      r.mortgageDebtJan1.toFixed(2),
      r.accumulatedCapitalJan1.toFixed(2),
      r.propertyValueJan1.toFixed(2),
      r.endOfYearNetValue.toFixed(2),
      r.endOfYearNetValueReal.toFixed(2),
...
function runSelfTests() {
  const model = {
    ...
    propertyAppreciationSchedule: [{ effectiveFrom: "2025-01-01", appreciation: 0 }],
    nlInflationSchedule: [{ effectiveFrom: "2025-01-01", inflation: 0 }],
    ltInflationSchedule: [{ effectiveFrom: "2025-01-01", inflation: 0 }],
    downpayment: 59250,
    loanAmount: 144750,
    ...
    propertySaleDate: "",
    rateSchedule: [
      { effectiveFrom: "2025-08-25", euribor: 0.0208 },
      { effectiveFrom: "2026-01-01", euribor: 0.0212 },
    ],
    etfContributions: [],
    lumpSums: [],
  };
  ...
  console.assert(Math.abs(jan2026.accumulatedCapital - 60081.80) < 2, "Jan 1 2026 accumulated capital should include Nov and Dec principal repayments");
}
...
document.getElementById("addPropertyAppreciationRow")?.addEventListener("click", () => {
  addPropertyAppreciationRow({ effectiveFrom: `${Math.round(inputNumber("projectionStartYear"))}-01-01`, appreciation: 0 });
  calculate();
});

document.getElementById("addNlInflationRow")?.addEventListener("click", () => {
  addInflationRow("#nlInflationTable", { effectiveFrom: `${Math.round(inputNumber("projectionStartYear"))}-01-01`, inflation: 0 });
  calculate();
});

document.getElementById("addLtInflationRow")?.addEventListener("click", () => {
  addInflationRow("#ltInflationTable", { effectiveFrom: `${Math.round(inputNumber("projectionStartYear"))}-01-01`, inflation: 0 });
  calculate();
});
...
document.getElementById("resetChartSeries")?.addEventListener("click", () => {
  chartSeries.forEach((s) => { s.enabled = true; });
  renderChartControls();
  if (window.__lastResult) renderChart(window.__lastResult.annual);
});
...
addPropertyAppreciationRow({ effectiveFrom: "2025-01-01", appreciation: 0 });
addInflationRow("#nlInflationTable", { effectiveFrom: "2025-01-01", inflation: 0 });
addInflationRow("#ltInflationTable", { effectiveFrom: "2025-01-01", inflation: 0 });

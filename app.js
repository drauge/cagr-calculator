...
const chartSeries = [
  ...
  { key: "endOfYearNetValue", label: "End-of-year net value", color: "#9467bd", enabled: true },
];
...
function addRateRow({ effectiveFrom = "2025-01-01", euribor = 3.5 } = {}) {
...
function readModel() {
  ...
  return {
    ...
    mortgageStartDate: document.getElementById("mortgageStartDate").value,
    durationMonths: Math.max(1, Math.round(inputNumber("durationMonths"))),
    interestOnlyMonths: Math.max(0, Math.round(inputNumber("interestOnlyMonths"))),
...
function activeEuribor(model, year, month) {
  const current = ymToDate(year, month).toISOString().slice(0, 10);
  let active = model.rateSchedule[0]?.euribor ?? 0;
  for (const row of model.rateSchedule) {
    if (row.effectiveFrom <= current) active = row.euribor;
  }
  return active;
}
...
function buildMortgageSchedule(model) {
  ...
  let lastRateKey = null;
  let currentPayment = 0;

  for (let i = 0; i <= model.durationMonths; i += 1) {
    const dateString = addMonthsToDateString(model.mortgageStartDate, i);
    const d = new Date(dateString + "T00:00:00Z");
    ...
    records.push({
      ...
      principalPaidCumulative: Math.max(0, model.loanAmount - balance),
      accumulatedCapital: model.downpayment + Math.max(0, model.loanAmount - balance),
    });

    if (i === model.durationMonths) break;

    const euribor = activeEuribor(model, year, month);
    const annualRate = euribor + model.bankMargin;
    const monthlyRate = annualRate / 12;
    const remainingMonths = model.durationMonths - i;

    const rateResetIndex = Math.floor(i / 6);
    const rateKey = `${rateResetIndex}-${annualRate.toFixed(8)}`;
    ...
    let principal = i < model.interestOnlyMonths ? 0 : currentPayment - interest;
    principal = Math.max(0, Math.min(balance, principal));
...
function mortgageSnapshotJan1(mortgageSchedule, year, model) {
  const exact = mortgageSchedule.find((r) => r.year === year && r.month === 1);
  if (exact) return exact;

  const start = new Date(model.mortgageStartDate + "T00:00:00Z");
  if (ymToDate(year, 1) < start) {
    return {
  ...
  const last = mortgageSchedule[mortgageSchedule.length - 1];
  return {
    balanceStart: last.balanceStart,
    accumulatedCapital: last.accumulatedCapital,
  };
}
...
function renderChart(rows) {
  ...
  enabled.forEach((series) => {
    const points = rows.map((r) => ({ x: xScale(r.year), y: yScale(r[series.key]), value: r[series.key], year: r.year }));
...
function downloadCsv() {
  ...
  a.href = url;
  a.download = "cagr_box3_mortgage_projection.csv";
  a.click();
...
function runSelfTests() {
  const model = {
    ...
    bankMargin: 0.0157,
    rateSchedule: [{ effectiveFrom: "2025-01-01", euribor: 0.035 }],
    etfContributions: [],
    lumpSums: [],
  };

  const tax = calculateBox3(model, 55000, 142000, 2025);
  console.assert(Math.abs(tax.taxBeforeRelief - 2299.05) < 3, "Box 3 tax before relief should match prior hand calculation approximately");
  console.assert(Math.abs(tax.finalTax - 630.06) < 5, "Final NL tax should match prior hand calculation approximately");

  const noTax = calculateBox3({ ...model, box3Allowance: 1000000 }, 55000, 142000, 2025);
  console.assert(noTax.finalTax === 0, "Very high allowance should produce zero final tax");
}
...
document.getElementById("addRateRow").addEventListener("click", () => {
  addRateRow({ effectiveFrom: `${inputNumber("projectionStartYear")}-01-01`, euribor: 3.5 });
  calculate();
});
document.getElementById("addEtfContribution").addEventListener("click", () => {
  const start = Math.round(inputNumber("projectionStartYear"));
  addEtfContributionRow({ amount: 0, frequency: "Yearly", month: 1, startYear: start, endYear: start + Math.round(inputNumber("projectionYears")) - 1 });
  calculate();
});
...
// Defaults based on this conversation.
addRateRow({ effectiveFrom: "2025-01-01", euribor: 3.5 });

...
function monthsBetweenDates(startDateString, endDateString) {
  return monthIndex(endDateString) - monthIndex(startDateString);
}
...
function readModel() {
  ...
  return {
    ...
    repaymentStartDate: document.getElementById("repaymentStartDate").value,
...
function buildPaymentDates(model) {
  ...
  for (let i = 0; i < model.durationMonths; i += 1) {
    if (i === 0) {
      ...
      dates.push(addMonthsToDateString(model.firstPaymentDate, i));
    }
  }
  return dates;
}
...
function mortgageSnapshotJan1(mortgageSchedule, year, model) {
  ...
  if (lastBeforeJan1) {
    return {
      ...
      accumulatedCapital: model.downpayment + Math.max(0, model.loanAmount - lastBeforeJan1.balanceEnd),
    };
  }
...
function runSelfTests() {
  ...
  console.assert(schedule[0].principal === 0, "First payment should be interest-only");
  console.assert(schedule[1].principal === 0, "Second payment should be interest-only");
  console.assert(schedule[2].principal > 0, "Third payment should start principal repayment");
  console.assert(Math.abs(schedule[2].payment - 855.55) < 2, "First annuity payment should be near contract schedule amount");

  const tax = calculateBox3(model, 55000, 142000, 2025);
  console.assert(Math.abs(tax.finalTax - 630.06) < 10, "Final NL tax should match prior hand calculation approximately");
}
...
document.getElementById("addRateRow").addEventListener("click", () => {
  addRateRow({ effectiveFrom: document.getElementById("mortgageStartDate").value, euribor: 2.08 });
  calculate();
});
